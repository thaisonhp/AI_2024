import cv2
from data_augumentation import SampleImgTransformer

# Đọc ảnh từ file
image = cv2.imread('/Users/luongthaison/Documents/Third_years_student/AI_AnhHung/data_augument_yolo/data/vu.jpeg')
cv2.imshow('Original Image', image)
cv2.waitKey(0)
cv2.destroyAllWindows()

# Khởi tạo đối tượng với ảnh gốc
transformer = SampleImgTransformer(image)
cv2.imshow('Original Image 2', transformer.original_image)
cv2.waitKey(0)
cv2.destroyAllWindows()

# Áp dụng biến đổi cho ảnh
transformer.apply_transformations(
    noise_type='gaussian',     # Thêm noise Gaussian
    rotation_angle=90,         # Xoay ảnh 90 độ
    brightness_scale=1.5,      # Thay đổi độ sáng
    perspective_angle=5         # Biến đổi phối cảnh với góc tối đa 5
)

# Lấy lại ảnh gốc
transformer.reset_image()
cv2.imshow('Reset Image', transformer.modified_image)
cv2.waitKey(0)
cv2.destroyAllWindows()
